...
const CodingAssessment = () => {
  ...
  const gazerIntervalId = useRef(null);
  ...
  useEffect(()=>{
    ...
    return () => {
      ...
      gazerIntervalId && gazerIntervalId.current && clearInterval(gazerIntervalId.current);
  ...
  const startTest = () => {
    audioIntervalId.current = setInterval(volumeCallback.current, 10000);
    gazerIntervalId.current = setInterval(()=>{
