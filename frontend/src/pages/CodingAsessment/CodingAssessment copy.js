...
const CodingAssessment = () => {
  ...
  const [lastSSTakenAt, setLastSSTakenAt] = useState(new Date());
  ...
  const takeScreenshot = async (warningType) => {
    const now = new Date();
    const lastTime = lastSSTakenAt;
    lastTime.setMinutes(lastTime.getMinutes() + 1);
    if(lastTime <= now && warningType != 'VOICE_DETECTED'){
      let blob = await imageCapture.current.takePhoto();
      let file = new File([blob], `${user.usn}_${screenshots.length + 1}.jpg`);
      setScreenshots([...screenshots, file]);
      setLastSSTakenAt(now);
    }
  ...
  if(circleCount == 20 && irisWarning){
    takeScreenshot('LOOKING_AWAY_FROM_SCREEN');
  }

  if(circleCount == 20 && audioWarning){
    takeScreenshot('VOICE_DETECTED');
  }
  ...
  const startPrediction = async () => {
    ...
    if(phone)
      takeScreenshot('PHONE_DETECTED');
    else if(count == 0)
      takeScreenshot('NO_PERSON_DETECTED');
    else if(count > 1)
      takeScreenshot('MULTIPLE_PERSONS_DETECTED');
  ...
  const recognizeFaces = async () => {
    ...
    if(!faceRecognized)
      takeScreenshot('FACE_NOT_RECOGNIZED');
    requestAnimationFrame(recognizeFaces);
  }

  const startTest = () => {
    audioIntervalId.current = setInterval(volumeCallback.current, 1000);
    startPrediction();
...
const Assessment = ({stream, startTest, endTest, testId, user}) => {
  ...
  return (
    <Box>
      {loading && 'Please wait while we submit your test ...'}
      {!loading && (
        <FullScreen
